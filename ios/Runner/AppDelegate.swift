...
@objc
class AppDelegate: FlutterAppDelegate, OPPCheckoutProviderDelegate, SFSafariViewControllerDelegate,
  PKPaymentAuthorizationViewControllerDelegate, OPPThreeDSEventListener
{
  ...
  override func application(
    ...
    let batteryChannel = FlutterMethodChannel(
      name: "Hyperpay.demo.fultter/channel",
    ...
    DispatchQueue.main.async {
      ...
      if self.brand == "mada" {
        ...
        let paymentRequest = OPPPaymentProvider.paymentRequest(
          withMerchantIdentifier: "merchant.applepaymosab", countryCode: "SA")
        if #available(iOS 12.1.1, *) {
          paymentRequest.supportedNetworks = [
            PKPaymentNetwork.mada, PKPaymentNetwork.visa,
      ...
      self.checkoutProvider = OPPCheckoutProvider(
        paymentProvider: self.provider, checkoutID: checkoutId, settings: checkoutSettings)!
        ...
        forSubmittingTransactionCompletionHandler: { (transaction, error) in
          ...
          if transaction.type == .synchronous {
            ...
            NotificationCenter.default.addObserver(
              self, selector: #selector(self.didReceiveAsynchronousPaymentCallback),
              name: Notification.Name(rawValue: "AsyncPaymentCompletedNotificationKey"), object: nil
            )
    ...
    if self.STCPAY == "enabled" {
      ...
      if self.brand == "APPLEPAY" {
        ...
        let request = OPPPaymentProvider.paymentRequest(
          withMerchantIdentifier: "merchant.applepaytest", countryCode: "SA")
        ...
        request.paymentSummaryItems = [
          PKPaymentSummaryItem(label: "Hyperpay", amount: NSDecimalNumber(value: self.amount))
        ]
        ...
        do {
          let params = try OPPTokenPaymentParams(
            checkoutID: checkoutId, tokenID: self.token, cardPaymentBrand: " ", cvv: self.cvv)
        ...
        do {
          ...
          let params = try OPPCardPaymentParams(
            checkoutID: checkoutId, paymentBrand: self.brands, holder: self.holder,
            number: self.number, expiryMonth: self.month, expiryYear: self.year, cvv: self.cvv)
  ...
  @objc func didReceiveAsynchronousPaymentCallback(result: @escaping FlutterResult) {
    NotificationCenter.default.removeObserver(
      self, name: Notification.Name(rawValue: "AsyncPaymentCompletedNotificationKey"), object: nil)
  ...
  func paymentAuthorizationViewController(
    ...
    if let params = try? OPPApplePayPaymentParams(
      checkoutID: self.checkoutid, tokenData: payment.token.paymentData)
      as OPPApplePayPaymentParams?
    {

      self.transaction = OPPTransaction(paymentParams: params)

      self.provider.submitTransaction(
        OPPTransaction(paymentParams: params),
